...
class FileRow(discord.ui.ActionRow):
    ...
    async def file_button(self, interaction: discord.Interaction["UniverseDexBot"], _):
        await interaction.response.send_message(file=text_to_file(self.content))


async def build_eval_response(
    ctx: commands.Context["UniverseDexBot"], content: str, *, time_taken: float, error: bool = False
) -> LayoutView | None:
    """
