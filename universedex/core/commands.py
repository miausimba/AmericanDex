...
from universedex.core.dev import send_interactive
from universedex.core.discord import LayoutView, View
from universedex.core.utils.formatting import pagify
from universedex.core.utils.menus import Menu, TextFormatter, TextSource
from bd_models.models import Ball
from settings.models import load_settings, settings

log = logging.getLogger("universedex.core.commands")

if TYPE_CHECKING:
    from admin_panel.logging import DequeHandler

    from .bot import UniverseDexBot
...
class SimpleCheckView(View):
    ...
    async def confirm_button(self, interaction: discord.Interaction["UniverseDexBot"], button: discord.ui.Button):
...
class Core(commands.Cog):
    """
    Core commands of UniverseDex bot
    """
    ...
    async def reload_package(self, package: str, *, with_prefix=False):
        try:
            ...
            if not with_prefix:
                await self.reload_package("universedex.packages." + package, with_prefix=True)
                return
            raise
