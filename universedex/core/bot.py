...
from universedex.core.commands import Core
from universedex.core.dev import Dev
from universedex.core.help import HelpCommand
from universedex.core.metrics import PrometheusServer
from universedex.core.utils.checks import check_perms
...
log = logging.getLogger("universedex.core.bot")
http_counter = Histogram("discord_http_requests", "HTTP requests", ["key", "code"])
impersonations: dict[int, discord.Member] = {}

DEFAULT_PACKAGES = (
    ("admin", "universedex.packages.admin"),
    ("balls", "universedex.packages.balls"),
    ("guildconfig", "universedex.packages.guildconfig"),
    ("countryballs", "universedex.packages.countryballs"),
    ("info", "universedex.packages.info"),
    ("players", "universedex.packages.players"),
    ("trade", "universedex.packages.trade"),
)


def owner_check(ctx: commands.Context[UniverseDexBot]):
...
class Translator(app_commands.Translator):
    async def translate(self, string: locale_str, locale: Locale, context: TranslationContextTypes) -> str | None:
        text = (
            ...
            .replace("UniverseDex", settings.bot_name)
        )
...
class CommandTree[Bot: UniverseDexBot](app_commands.CommandTree[Bot]):
...
class UniverseDexBot(commands.AutoShardedBot):
    """
    ...
    def __init__(
        self,
        command_prefix: PrefixType[UniverseDexBot],
