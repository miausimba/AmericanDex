...
from universedex.core.discord import Modal, View
from universedex.core.metrics import caught_balls
from universedex.core.utils.utils import can_mention
...
class CountryballNamePrompt(Modal, title=f"Catch this {settings.collectible_name}!"):
    ...
    async def on_error(self, interaction: discord.Interaction["UniverseDexBot"], error: Exception) -> None:
    ...
    async def on_submit(self, interaction: discord.Interaction["UniverseDexBot"]):
...
class BallSpawnView(View):
    ...
    countryball in the UniverseDex bot. It handles user interactions, spawning mechanics, and
    ...
    def __init__(self, bot: "UniverseDexBot", model: Ball):
        super().__init__()
        self.bot = bot
        self.model = model
    ...
    async def catch_button(self, interaction: discord.Interaction["UniverseDexBot"], button: Button):
        if self.caught:
            slow_message = settings.get_random_message(PromptMessage.PromptType.SLOW).format(
                user=interaction.user.mention,
                collectible=settings.collectible_name,
    ...
    @classmethod
    async def from_existing(cls, bot: "UniverseDexBot", ball_instance: BallInstance):
        """
    ...
    @classmethod
    async def get_random(cls, bot: "UniverseDexBot"):
        """
