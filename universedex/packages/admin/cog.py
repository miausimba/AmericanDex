...
from universedex.core.bot import impersonations
from universedex.core.discord import LayoutView
from universedex.core.utils import checks
from universedex.core.utils.buttons import ConfirmChoiceView
from universedex.core.utils.menus import (
    ItemFormatter,
    ListSource,
    Menu,
    TextFormatter,
...
if TYPE_CHECKING:
    ...
    from universedex.packages.countryballs.cog import CountryBallsSpawner
    from universedex.packages.trade.cog import Trade

log = logging.getLogger("universedex.packages.admin")
...
class SyncView(LayoutView):
    ...
    async def sync(self, interaction: discord.Interaction["UniverseDexBot"], button: Button):
    ...
    async def remove(self, interaction: discord.Interaction["UniverseDexBot"], button: Button):
        assert interaction.guild
        self.stop()
        await interaction.response.defer()
...
class Admin(commands.Cog):
    ...
    def __init__(self, bot: "UniverseDexBot"):
        self.bot = bot
    ...
    async def cog_check(self, ctx: commands.Context["UniverseDexBot"]) -> bool:
        return await checks.is_staff().predicate(ctx)

    async def cog_app_command_error(
        self, interaction: discord.Interaction["UniverseDexBot"], error: app_commands.AppCommandError
    ):
    ...
    @commands.is_owner()
    @commands.guild_only()
    async def syncslash(self, ctx: commands.Context["UniverseDexBot"]):
        """
    ...
    @admin.command()
    @checks.is_superuser()
    async def status(self, ctx: commands.Context["UniverseDexBot"], *, flags: StatusFlags):
        """
    ...
    @admin.command()
    @checks.is_superuser()
    async def trade_lockdown(self, ctx: commands.Context["UniverseDexBot"], *, reason: str):
        """
    ...
    async def rarity(self, ctx: commands.Context["UniverseDexBot"], *, flags: RarityFlags):
        """
    ...
    @admin.command()
    @checks.is_superuser()
    async def cooldown(self, ctx: commands.Context["UniverseDexBot"], guild_id: str | None = None):
        """
    ...
    @admin.command()
    async def guilds(self, ctx: commands.Context["UniverseDexBot"], user: discord.User):
        """
    ...
    @admin.command()
    @checks.is_superuser()
    async def impersonate(self, ctx: commands.Context["UniverseDexBot"], user: discord.Member | None = None):
        """
