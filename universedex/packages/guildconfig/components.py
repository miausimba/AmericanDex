...
class AcceptTOSView(View):
    ...
    def __init__(
        self, interaction: discord.Interaction["UniverseDexBot"], channel: discord.TextChannel, new_player: discord.Member
    ):
        super().__init__()
    ...
    async def accept_button(self, interaction: discord.Interaction["UniverseDexBot"], button: discord.ui.Button):
        ...
        config.enabled = True
        await config.asave()
        interaction.client.dispatch("universedex_settings_change", interaction.guild, channel=self.channel, enabled=True)
        self.stop()
        if self.message:
