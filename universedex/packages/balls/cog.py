...
from universedex.core.discord import View
from universedex.core.utils.buttons import ConfirmChoiceView
from universedex.core.utils.menus import ChunkedListSource, Menu, SelectFormatter, TextFormatter, TextSource
from universedex.core.utils.sorting import FilteringChoices, SortingChoices, filter_balls, sort_balls
from universedex.core.utils.transformers import (
    BallEnabledTransform,
    BallInstanceTransform,
    SpecialEnabledTransform,
    TradeCommandType,
)
from universedex.core.utils.utils import can_mention, inventory_privacy, is_staff
...
log = logging.getLogger("universedex.packages.countryballs")
...
class DonationRequest(View):
    def __init__(
        self,
        bot: "UniverseDexBot",
        interaction: discord.Interaction["UniverseDexBot"],
    ...
    async def interaction_check(self, interaction: discord.Interaction["UniverseDexBot"], /) -> bool:
    ...
    async def accept(self, interaction: discord.Interaction["UniverseDexBot"], button: Button):
        self.stop()
        for item in self.children:
            item.disabled = True  # type: ignore
    ...
    async def deny(self, interaction: discord.Interaction["UniverseDexBot"], button: Button):
        self.stop()
        for item in self.children:
            item.disabled = True  # type: ignore
        await interaction.response.edit_message(
...
class Balls(commands.GroupCog, group_name=settings.balls_slash_name):
    ...
    async def completion(
        ...
        """
        Show your current completion of the UniverseDex.
    ...
    @app_commands.checks.cooldown(1, 20, key=lambda i: i.user.id)
    async def duplicate(
        self, interaction: discord.Interaction["UniverseDexBot"], type: DuplicateType, limit: int | None = None
    ):
        """
