...
from americandex.core.bot import impersonations
from americandex.core.discord import LayoutView
from americandex.core.utils import checks
from americandex.core.utils.buttons import ConfirmChoiceView
from americandex.core.utils.menus import (
    ItemFormatter,
    ListSource,
    Menu,
    TextFormatter,
...
if TYPE_CHECKING:
    ...
    from americandex.packages.countryballs.cog import CountryBallsSpawner
    from americandex.packages.trade.cog import Trade

log = logging.getLogger("americandex.packages.admin")
...
class SyncView(LayoutView):
    ...
    async def sync(self, interaction: discord.Interaction["AmericanDexBot"], button: Button):
    ...
    async def remove(self, interaction: discord.Interaction["AmericanDexBot"], button: Button):
        assert interaction.guild
        self.stop()
        await interaction.response.defer()
...
class Admin(commands.Cog):
    ...
    async def cog_check(self, ctx: commands.Context["AmericanDexBot"]) -> bool:
        return await checks.is_staff().predicate(ctx)

    async def cog_app_command_error(
        self, interaction: discord.Interaction["AmericanDexBot"], error: app_commands.AppCommandError
    ):
    ...
    @commands.is_owner()
    @commands.guild_only()
    async def syncslash(self, ctx: commands.Context["AmericanDexBot"]):
        """
    ...
    @admin.command()
    @checks.is_superuser()
    async def status(self, ctx: commands.Context["AmericanDexBot"], *, flags: StatusFlags):
        """
    ...
    @admin.command()
    @checks.is_superuser()
    async def trade_lockdown(self, ctx: commands.Context["AmericanDexBot"], *, reason: str):
        """
    ...
    async def rarity(self, ctx: commands.Context["AmericanDexBot"], *, flags: RarityFlags):
        """
    ...
    @admin.command()
    @checks.is_superuser()
    async def cooldown(self, ctx: commands.Context["AmericanDexBot"], guild_id: str | None = None):
        """
    ...
    @admin.command()
    async def guilds(self, ctx: commands.Context["AmericanDexBot"], user: discord.User):
        """
    ...
    @admin.command()
    @checks.is_superuser()
    async def impersonate(self, ctx: commands.Context["AmericanDexBot"], user: discord.Member | None = None):
        """
