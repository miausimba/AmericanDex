...
from americandex.core.utils.menus import ChunkedListSource, Menu, SelectFormatter, TextFormatter, TextSource
from americandex.core.utils.sorting import FilteringChoices, SortingChoices, filter_balls, sort_balls
from americandex.core.utils.transformers import (
    BallEnabledTransform,
    BallInstanceTransform,
    SpecialEnabledTransform,
    TradeCommandType,
)
from americandex.core.utils.utils import can_mention, inventory_privacy, is_staff
...
log = logging.getLogger("americandex.packages.countryballs")
...
class DonationRequest(View):
    def __init__(
        self,
        bot: "AmericanDexBot",
        interaction: discord.Interaction["AmericanDexBot"],
    ...
    async def interaction_check(self, interaction: discord.Interaction["AmericanDexBot"], /) -> bool:
    ...
    async def accept(self, interaction: discord.Interaction["AmericanDexBot"], button: Button):
        self.stop()
        for item in self.children:
            item.disabled = True  # type: ignore
    ...
    async def deny(self, interaction: discord.Interaction["AmericanDexBot"], button: Button):
        self.stop()
        for item in self.children:
            item.disabled = True  # type: ignore
        await interaction.response.edit_message(
...
class Balls(commands.GroupCog, group_name=settings.balls_slash_name):
    ...
    async def completion(
        ...
        """
        Show your current completion of the AmericanDex.
    ...
    @app_commands.checks.cooldown(1, 20, key=lambda i: i.user.id)
    async def duplicate(
        self, interaction: discord.Interaction["AmericanDexBot"], type: DuplicateType, limit: int | None = None
    ):
        """
