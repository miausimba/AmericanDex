...
from americandex.core.discord import LayoutView, View
from americandex.core.utils.menus import ChunkedListSource, Menu, SelectFormatter
...
class ExtraLicenseView(LayoutView):
    header = TextDisplay(
        "This instance of Americandex is powered by 3rd-party packages whose information can be found below."
    )
    row = ActionRow()

    @row.select()
    async def extra_package_select(self, interaction: discord.Interaction["AmericanDexBot"], select: Select):
...
class LicenseInfo(View):
    ...
    async def license_info(self, interaction: discord.Interaction["AmericanDexBot"], _: Button):
        await interaction.response.send_message(
            "This bot is an instance of AmericanDex-DiscordBot "
            "(hereinafter referred to as Americandex).\n"
            "Americandex is a free and open source application made available to the public and "
            "licensed under the MIT license. The full text of this license is attached below.\n",
            ephemeral=True,
            file=discord.File(
                get_license_files(importlib.metadata.distribution("americandex"))[0].locate(), filename="LICENSE.txt"
            ),
        )

    @button(label="3rd party packages")
    async def extra_packages(self, interaction: discord.Interaction["AmericanDexBot"], _: Button):
