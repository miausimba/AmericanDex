...
class AcceptTOSView(View):
    ...
    def __init__(
        self, interaction: discord.Interaction["AmericanDexBot"], channel: discord.TextChannel, new_player: discord.Member
    ):
        super().__init__()
    ...
    async def accept_button(self, interaction: discord.Interaction["AmericanDexBot"], button: discord.ui.Button):
        ...
        config.enabled = True
        await config.asave()
        interaction.client.dispatch("americandex_settings_change", interaction.guild, channel=self.channel, enabled=True)
        self.stop()
        if self.message:
