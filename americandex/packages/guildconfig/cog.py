...
class Config(commands.GroupCog):
    ...
    async def channel(
        self, interaction: discord.Interaction["AmericanDexBot"], channel: Optional[discord.TextChannel] = None
    ):
        """
    ...
    async def disable(self, interaction: discord.Interaction["AmericanDexBot"]):
        """
        ...
        if config.enabled:
            ...
            await config.asave()
            self.bot.dispatch("americandex_settings_change", guild, enabled=False)
            await interaction.response.send_message(
            ...
            await config.asave()
            self.bot.dispatch("americandex_settings_change", guild, enabled=True)
    ...
    @app_commands.command()
    @app_commands.checks.has_permissions(manage_guild=True)
    async def status(self, interaction: discord.Interaction["AmericanDexBot"]):
        """
