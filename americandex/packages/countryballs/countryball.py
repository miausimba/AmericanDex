...
from americandex.core.discord import Modal, View
from americandex.core.metrics import caught_balls
from americandex.core.utils.utils import can_mention
...
class CountryballNamePrompt(Modal, title=f"Catch this {settings.collectible_name}!"):
    ...
    async def on_error(self, interaction: discord.Interaction["AmericanDexBot"], error: Exception) -> None:
    ...
    async def on_submit(self, interaction: discord.Interaction["AmericanDexBot"]):
...
class BallSpawnView(View):
    ...
    countryball in the AmericanDex bot. It handles user interactions, spawning mechanics, and
    ...
    def __init__(self, bot: "AmericanDexBot", model: Ball):
        super().__init__()
        self.bot = bot
        self.model = model
    ...
    async def catch_button(self, interaction: discord.Interaction["AmericanDexBot"], button: Button):
        if self.caught:
            slow_message = settings.get_random_message(PromptMessage.PromptType.SLOW).format(
                user=interaction.user.mention,
                collectible=settings.collectible_name,
    ...
    @classmethod
    async def from_existing(cls, bot: "AmericanDexBot", ball_instance: BallInstance):
        """
    ...
    @classmethod
    async def get_random(cls, bot: "AmericanDexBot"):
        """
