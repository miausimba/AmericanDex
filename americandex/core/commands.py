...
from americandex.core.dev import send_interactive
from americandex.core.discord import LayoutView, View
from americandex.core.utils.formatting import pagify
from americandex.core.utils.menus import Menu, TextFormatter, TextSource
from bd_models.models import Ball
from settings.models import load_settings, settings

log = logging.getLogger("americandex.core.commands")

if TYPE_CHECKING:
    from admin_panel.logging import DequeHandler

    from .bot import AmericanDexBot
...
class SimpleCheckView(View):
    ...
    async def confirm_button(self, interaction: discord.Interaction["AmericanDexBot"], button: discord.ui.Button):
...
class Core(commands.Cog):
    """
    Core commands of AmericanDex bot
    """
    ...
    async def reload_package(self, package: str, *, with_prefix=False):
        try:
            ...
            if not with_prefix:
                await self.reload_package("americandex.packages." + package, with_prefix=True)
                return
            raise
