...
from americandex.core.commands import Core
from americandex.core.dev import Dev
from americandex.core.help import HelpCommand
from americandex.core.metrics import PrometheusServer
from americandex.core.utils.checks import check_perms
...
log = logging.getLogger("americandex.core.bot")
http_counter = Histogram("discord_http_requests", "HTTP requests", ["key", "code"])
impersonations: dict[int, discord.Member] = {}

DEFAULT_PACKAGES = (
    ("admin", "americandex.packages.admin"),
    ("balls", "americandex.packages.balls"),
    ("guildconfig", "americandex.packages.guildconfig"),
    ("countryballs", "americandex.packages.countryballs"),
    ("info", "americandex.packages.info"),
    ("players", "americandex.packages.players"),
    ("trade", "americandex.packages.trade"),
)


def owner_check(ctx: commands.Context[AmericanDexBot]):
...
class Translator(app_commands.Translator):
    async def translate(self, string: locale_str, locale: Locale, context: TranslationContextTypes) -> str | None:
        text = (
            ...
            .replace("AmericanDex", settings.bot_name)
        )
...
class CommandTree[Bot: AmericanDexBot](app_commands.CommandTree[Bot]):
...
class AmericanDexBot(commands.AutoShardedBot):
    """
    ...
    def __init__(
        self,
        command_prefix: PrefixType[AmericanDexBot],
