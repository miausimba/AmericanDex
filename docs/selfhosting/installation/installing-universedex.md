Universedex is an open source Discord bot for catching countryballs, which you can install yourself and customize!

While it can run on all operating systems, it is highly recommended to host Universedex on Linux (macOS mostly works the same as well). The recommended distribution we will use through this tutorial is **Ubuntu 24.04**.
...
To run Universedex, we need Docker and git.
...
```bash { .copy }
git clone https://github.com/laggron42/UniverseDex-DiscordBot.git
```

Then you can use the command `cd` to change directory and open the folder you just downloaded:
```bash { .copy }
cd UniverseDex-DiscordBot
```

From this point, every time you need to type commands for Universedex, **you must always open the bot's directory first**.
...
3. "Bot name" is used in various places like `/about` or `/universe completion`.
...
To start the bot, simply run `docker compose up`. This will both start the bot and the admin panel, while showing you the live logs. Wait until the line "Universedex bot is now ready" shows up, and the bot should be online!
