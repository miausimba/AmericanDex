# Installing Americandex without Docker

The recommended way to run Americandex is using Docker, but you may have reasons to run it without this dependency.
...
!!! tip
    Americandex requires Python 3.14 at least.
...
3.  Export the `AMERICANDEXBOT_DB_URL` environment variable with the link to Postgres you tested earlier.
...
Follow [this section](installing-americandex.md#5-configure-the-bot) from the main tutorial to fill the base settings.
...
!!! warning
    ...
    The default configuration does not handle static files and should be handled by you. Americandex
