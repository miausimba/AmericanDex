This guide will help you upgrade from 2.X to Universedex 3.0.

!!! warning
    Universedex 3.0 brings a lot of breaking changes!
...
=== "Without Docker"
    ...
    ```bash
    pg_dump -U universedex universedex -f pre-migration-data-dump.sql
    ```
...
=== "Without Docker"
    ...
    ```bash
    # activate your venv and export UNIVERSEDEXBOT_DB_URL
    cd admin_panel
    ...
    pg_dump -U universedex universedex -f final-data-dump.sql
    ```
...
=== "With Docker"
    ...
    3.  Import your database dump
        ```bash
        cat final-data-dump.sql | docker compose exec -T postgres-db psql -U universedex universedex
        ```
    4.  Rebuild the bot
        ```bash
...
=== "Without Docker"
    ...
    3.  Install the new virtual environment
        ...
        ```
        !!! note
            Python 3.14 is required to run Universedex 3.0. If it's not installed on your system,
...
Universedex 3.0 does not use `config.yml` anymore for its configuration, instead it's all on the
