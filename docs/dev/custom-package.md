...
This tutorial will explain you how to extend Universedex with additional commands and features
the recommended way.
...
1.  Download [Visual Studio Code](https://code.visualstudio.com/). Make sure to check the
    ...
    !!! info
        You can use the editor of your choice, but Universedex is pre-configured for VScode and will
        give you the best out-of-the-box experience.

2.  Install and configure Universedex locally. I recommend that you follow the
    [Dockerless install](/selfhosting/installation/installing-universedex-no-docker/) to enable
    ...
    2.  Open a terminal and `cd` into the Universedex folder
    3.  Run `uv sync --all-extras`

        You may need to run this command from time to time if dependencies are updated.

3.  Open a terminal, `cd` into the Universedex folder and run `code .` to open Visual Studio Code.
...
4.  Now you can run the bot in debug mode
    ...
    === "With Docker"
        ...
        services:
          bot:
            command: python3 -m universedex --dev --debug
            environment:
    ...
    === "Without Docker"
        ...
        python3 -m universedex --dev --debug
...
That's it, you're now setup to develop on Universedex!
...
2.  Since you are writing a full Python package, you need a `pyproject.toml` file. Create the file
    ...
    dependencies = [
        # you can require a specific version of universedex here
        "universedex>=3.0.0",
    ]
    ```
...
1.  Let's call our extension `my_cool_ext`. We will create the following files:
    ...
    class YourCog(commands.Cog):
        ...
        @commands.command()
        async def hello(self, ctx: commands.Context["UniverseDexBot"]):
...
```
2025-12-16 14:32:17 INFO     universedex.core.bot Packages loaded: admin, balls, guildconfig, countryballs, info, players, trade, my_cool_ext
```
...
class YourCog(commands.Cog):
    ...
    async def profile(self, interaction: discord.Interaction["UniverseDexBot"], user: discord.User | None = None):
...
5.  Review the contents of your `pyproject.toml` and add some metadata. Don't forget to put the
    ...
    authors = [
        { name = "laggron42", email = "laggron42@universedex.com" },
    ]
    readme = "README.md"
